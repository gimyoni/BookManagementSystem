...
public class DeleteUser extends JFrame { //회원탈퇴를 위한 클래스 생성
	...
	private JPanel contentPane; //contentPane 생성
	private JTextField textField; //textField 생성
	private JPasswordField passwordField; //passwordField 생성
	Connection conn;
	
	public DeleteUser() { //DeleteUser 메소드
		setTitle("\uD68C\uC6D0\uD0C8\uD1F4");
		conn = DBManager.getConnection();
		
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		setBounds(100, 100, 450, 300); //전체 창의 크기
		...
		JLabel label = new JLabel("ID"); //ID Label
		...
		JLabel label_1 = new JLabel("PASSWORD"); //Password Label
		...
		JButton btnNewButton = new JButton("Back"); //뒤로가기 버튼
			...
			try {
				if(rs.next()) {
					...
					if(password.contentEquals(password)) {
						sql="delete from client where id =?";  //client에서 id가 ? 인 값을 삭제 
						pstmt = conn.prepareStatement(sql);
						pstmt.setString(1, id); //물음표의 값지정
						pstmt.executeUpdate();
